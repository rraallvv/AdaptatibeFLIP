...
void bcc2::cleanupNodeAndElements() {
	std::unordered_map<uint64,uint> remap;
...
#endif
	uint index=0;
	std::unordered_map<uint64,vec2i>::iterator it = nodes.begin();
	while( it != nodes.end() ) {
...
#if CENTERS_DEF
	std::unordered_map<uint64,bool>::iterator it2 = centers.begin();
	...
	std::unordered_map<uint64,vec2i>().swap(nodes);
#if CENTERS_DEF
	std::unordered_map<uint64,bool>().swap(centers);
#endif
